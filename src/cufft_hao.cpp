...
FFTServer_cu::FFTServer_cu()
{
    ...
    cudaMalloc((void**)&dataforw, sizeof(cufftDoubleComplex)*L*BATCH);
    cudaMalloc((void**)&databack, sizeof(cufftDoubleComplex)*L*BATCH);

#if USE_CUDA_HOST 
    cudaMallocHost((void**)&outforw_host, sizeof(cufftDoubleComplex)*L*BATCH);
    cudaMallocHost((void**)&outback_host, sizeof(cufftDoubleComplex)*L*BATCH);
#else
    outforw_host = new complex<double>[sizeof(cufftDoubleComplex)*L*BATCH];
    outback_host = new complex<double>[sizeof(cufftDoubleComplex)*L*BATCH];
#endif 

    stat = cufftPlanMany(&plan, dimen, n, NULL, 1, L, NULL, 1, L, CUFFT_Z2Z, BATCH);
...
FFTServer_cu::FFTServer_cu(int Dc, const int* Nc, char format, int batch)
{
    cufftResult stat;
    dimen = Dc;
    n = new int[dimen];
...
complex<double>* FFTServer_cu::fourier_forw(const complex<double>* inarray)
{
    ...
    cudaMemcpy(dataforw, inarray, sizeof(cufftDoubleComplex)*L*BATCH, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(outforw_host, dataforw, sizeof(cufftDoubleComplex)*L*BATCH, cudaMemcpyDeviceToHost);
...
complex<double>* FFTServer_cu::fourier_back(const complex<double>* inarray)
{
    ...
    cudaMemcpy(databack, inarray, sizeof(cufftDoubleComplex)*L*BATCH, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(outback_host, databack, sizeof(cufftDoubleComplex)*L*BATCH, cudaMemcpyDeviceToHost);
